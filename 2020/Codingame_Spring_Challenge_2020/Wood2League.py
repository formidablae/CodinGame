...
def isNearEnough(start_coordinate: Coordinate, end_coordinate: Coordinate, distance: int):
    return abs(start_coordinate.coordinateX - end_coordinate.coordinateX) + abs(start_coordinate.coordinateY - end_coordinate.coordinateY) < distance
...
def findMyPac(pacList: list):
    for pac in pacList:
        if pac.mineOrNot:
            return pac
    return Pac(-1, False, Coordinate(-1, -1))
...
def findNearestPacEnemy(pacList: list, myPacCoordinates: Coordinate):
    ...
    for pac in pacList:
        if not pac.mineOrNot:
            newDistance = min(distance, abs(myPacCoordinates.coordinateX - pac.pacCoordinate.coordinateX) + abs(myPacCoordinates.coordinateY - pac.pacCoordinate.coordinateY))
...
def checkEnemyPacPosition(pacList: list, myPacCoordinates: Coordinate):
    for pac in pacList:
        if not pac.mineOrNot:
            if abs(myPacCoordinates.coordinateX - pac.pacCoordinate.coordinateX) == 1 or abs(myPacCoordinates.coordinateY - pac.pacCoordinate.coordinateY) == 1:
                return Coordinate(myPacCoordinates.coordinateX - pac.pacCoordinate.coordinateX, myPacCoordinates.coordinateY - pac.pacCoordinate.coordinateY)
    return Coordinate(2, 2)


def findNearestPossiblePosInDirection(gridMatrix: list, myPacCoordinates: Coordinate, nearbyEnemyPacCoordinatesDifference: Coordinate):
    if nearbyEnemyPacCoordinatesDifference.coordinateX == 0:
        ...
        if isValid(Coordinate(myPacCoordinates.coordinateX + nearbyEnemyPacCoordinatesDifference.coordinateX,
                   ...
                   gridMatrix) and gridMatrix[myPacCoordinates.coordinateX + nearbyEnemyPacCoordinatesDifference.coordinateX][
            myPacCoordinates.coordinateY] == " ":
...
while True:
    ...
    myPacCoordinates: Coordinate = findMyPac(pacList).pacCoordinate
    ...
    if nearbyEnemyPacCoordinatesDifference.coordinateX > 1:
        ...
        # MOVE <pacId> <x> <y>
        print("MOVE 0 " + str(xCoord) + " " + str(yCoord))
    else:
        newCoordinates = findNearestPossiblePosInDirection(grid, myPacCoordinates, nearbyEnemyPacCoordinatesDifference)
        print("MOVE 0 " + str(newCoordinates.coordinateX) + " " + str(newCoordinates.coordinateY))
